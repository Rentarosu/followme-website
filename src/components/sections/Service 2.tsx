  ...
  return (
    <section id="service" className="relative py-28 md:py-36 bg-white overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-[480px] md:max-w-[1100px] px-6">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.7 }} className="mb-20">
        ...
        <div className="flex flex-col gap-16">
          {services.map((service, i) => (
            <motion.div key={service.number} initial={{ opacity: 0, x: i % 2 === 0 ? -60 : 60 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6, delay: i * 0.1 }} className={`flex flex-col ${i % 2 === 1 ? "md:flex-row-reverse" : "md:flex-row"} items-center gap-8 md:gap-12`}>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.7 }} className="mt-24 grid grid-cols-1 md:grid-cols-2 gap-6">
          <a href="#" className="group relative rounded-2xl p-8 md:p-10 overflow-hidden transition-all duration-300 hover:shadow-xl" style={{ background: "linear-gradient(135deg, #1B1A1F 0%, #2a2930 100%)" }}>
            <p className="text-xs font-bold text-primary tracking-[0.15em] uppercase mb-3">For Business</p>
            <h3 className="text-xl md:text-2xl font-black text-white mb-3">企業の方へ</h3>
            <p className="text-sm text-white/50 leading-relaxed mb-6">Hack UGCを活用して、貴社の商品・サービスのトレンドを生み出しませんか？プラットフォームの詳細をご紹介します。</p>
            <span className="inline-flex items-center gap-2 text-sm font-bold text-white group-hover:gap-3 transition-all duration-300">詳しくはこちら<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M3 8h10M9 4l4 4-4 4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></span>
          </a>
          <a href="#" className="group relative rounded-2xl p-8 md:p-10 overflow-hidden transition-all duration-300 hover:shadow-xl" style={{ background: "linear-gradient(135deg, #eef8ff 0%, #f3eeff 50%, #e8f4ff 100%)" }}>
            <p className="text-xs font-bold text-accent tracking-[0.15em] uppercase mb-3">For Creators</p>
            <h3 className="text-xl md:text-2xl font-black text-dark mb-3">クリエイターの方へ</h3>
            <p className="text-sm text-dark/50 leading-relaxed mb-6">大学生・Z世代のあなたの発信力を活かしませんか？クリエイターとして参加する方法をご紹介します。</p>
            <span className="inline-flex items-center gap-2 text-sm font-bold text-dark group-hover:gap-3 transition-all duration-300">詳しくはこちら<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M3 8h10M9 4l4 4-4 4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></span>
          </a>
        </motion.div>
      </div>
    </section>
