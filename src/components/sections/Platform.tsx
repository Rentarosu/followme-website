  ...
  return (
    <section id="platform" className="relative py-20 md:py-28 bg-white overflow-hidden">
      <div className="mx-auto max-w-[480px] md:max-w-[900px] px-6">
        ...
        <div className="animate-fade-in-up flex flex-col gap-8" style={{ animationDelay: "0.1s" }}>
          {/* For Business */}
          <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10 overflow-hidden">
            ...
            <div className="flex-1 px-6 pb-6 md:px-0 md:pr-8 md:py-8">
              ...
              <button onClick={() => setShowModal(true)} className="inline-flex items-center gap-3 text-white font-bold text-sm px-10 py-3 rounded-full border-2 border-white shadow-lg hover:scale-[1.04] hover:shadow-xl transition-all duration-300" style={{ background: "linear-gradient(135deg, #02a1f2 0%, #6ec4ff 100%)" }}>
                詳細はこちら
          ...
          <div className="flex flex-col md:flex-row-reverse items-center gap-6 md:gap-10 overflow-hidden">
            ...
            <div className="flex-1 px-6 pb-6 md:px-0 md:pl-8 md:py-8">
              ...
              <button onClick={() => setShowModal(true)} className="inline-flex items-center gap-3 text-white font-bold text-sm px-10 py-3 rounded-full border-2 border-white shadow-lg hover:scale-[1.04] hover:shadow-xl transition-all duration-300" style={{ background: "linear-gradient(135deg, #8B5CF6 0%, #A78BFA 100%)" }}>
                詳細はこちら
                <span className="w-7 h-7 rounded-full bg-white flex items-center justify-center flex-shrink-0">
      ...
      {showModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-6 transition-opacity duration-300"
          onClick={() => setShowModal(false)}
        >
          <div
            className="bg-white rounded-2xl p-8 md:p-12 max-w-md w-full text-center shadow-2xl animate-fade-in-up"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="text-4xl mb-4">🚧</div>
            <h3 className="text-xl font-black text-dark mb-3">ページ準備中</h3>
            <p className="text-sm text-dark/50 leading-relaxed mb-6">
              現在、ランディングページを制作中です。<br />
              公開まで今しばらくお待ちください。
            </p>
            <button
              onClick={() => setShowModal(false)}
              className="px-8 py-3 rounded-full bg-dark text-white font-bold text-sm hover:scale-[1.02] transition-all"
            >
              閉じる
            </button>
          </div>
